...
public:
  ...
  // distance function.
  unsigned char MaskedEdgeCase(unsigned char eCase) { return (eCase & this->InvalidMask); }
  ...
  {
    return (((*(ePtr[0])) & this->InvalidMask) | (((*(ePtr[1])) & this->InvalidMask) << 2) |
      (((*(ePtr[2])) & this->InvalidMask) << 4) | (((*(ePtr[3])) & this->InvalidMask) << 6));
  }
  ...
  void InterpolateAxesEdge(double t, unsigned char loc, float x0[3], T const* const s,
    const int incs[3], float x1[3], vtkIdType vId, vtkIdType ijk[3], float g0[3])
  {
    ...
    if (this->NeedGradients)
    {
      ...
      this->ComputeGradient(
        loc, ijk, s + incs[0], s - incs[0], s + incs[1], s - incs[1], s + incs[2], s - incs[2], g1);
  ...
  {
  public:
    Pass2(vtkExtractSurfaceAlgorithm<TT>* algo) { this->Algo = algo; }
    vtkExtractSurfaceAlgorithm<TT>* Algo;
    void operator()(vtkIdType slice, vtkIdType end)
    {
      for (; slice < end; ++slice)
  ...
  static void Contour(vtkExtractSurface* self, vtkImageData* input, int extent[6], vtkIdType* incs,
    T* scalars, vtkPoints* newPts, vtkCellArray* newTris, vtkFloatArray* newNormals,
...
template <class T>
const unsigned char vtkExtractSurfaceAlgorithm<T>::EdgeMap[12] = { 0, 5, 1, 4, 2, 7, 3, 6, 8, 9, 10,
  11 };

//------------------------------------------------------------------------------
// Map MC edges numbering to use the saner FlyingEdges edge numbering scheme.
template <class T>
const unsigned char vtkExtractSurfaceAlgorithm<T>::VertMap[12][2] = { { 0, 1 }, { 2, 3 }, { 4, 5 },
  { 6, 7 }, { 0, 2 }, { 1, 3 }, { 4, 6 }, { 5, 7 }, { 0, 4 }, { 1, 5 }, { 2, 6 }, { 3, 7 } };

//------------------------------------------------------------------------------
// The offsets of each vertex (in index space) from the voxel axes origin.
template <class T>
const unsigned char vtkExtractSurfaceAlgorithm<T>::VertOffsets[8][3] = { { 0, 0, 0 }, { 1, 0, 0 },
...
template <class T>
vtkExtractSurfaceAlgorithm<T>::vtkExtractSurfaceAlgorithm()
  : XCases(nullptr)
...
template <class T>
void vtkExtractSurfaceAlgorithm<T>::CountBoundaryYZInts(
...
// information.
template <class T>
void vtkExtractSurfaceAlgorithm<T>::ProcessYZEdges(vtkIdType row, vtkIdType slice)
{
...
// algorithm.
template <class T>
void vtkExtractSurfaceAlgorithm<T>::GenerateOutput(
  ...
  for (i = xL; i < xR; ++i)
  {
    ...
    if (numTris > 0)
    {
      ...
      if (this->CaseIncludesAxes(eCase) || loc != Interior)
      {
        ...
        this->GeneratePoints(value, loc, ijk, sPtr, incs, x, edgeUses, eIds);
      }
...
template <class T>
void vtkExtractSurfaceAlgorithm<T>::Contour(vtkExtractSurface* self, vtkImageData* input,
  int extent[6], vtkIdType* incs, T* scalars, vtkPoints* newPts, vtkCellArray* newTris,
...
//------------------------------------------------------------------------------
int vtkExtractSurface::RequestUpdateExtent(vtkInformation* vtkNotUsed(request),
  ...
  vtkInformation* request, vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  {
    vtkTemplateMacro(vtkExtractSurfaceAlgorithm<VTK_TT>::Contour(
      this, input, exExt, incs, (VTK_TT*)ptr, newPts, newTris, newNormals, newGradients));
  }
